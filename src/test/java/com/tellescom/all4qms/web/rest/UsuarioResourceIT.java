...
class UsuarioResourceIT {
    ...
    private static final Instant DEFAULT_CRIADO_EM = ZonedDateTime.ofInstant(Instant.ofEpochMilli(0L), ZoneOffset.UTC).toInstant();
    private static final Instant UPDATED_CRIADO_EM = ZonedDateTime.now(ZoneId.systemDefault()).withNano(0).toInstant();

    private static final Instant DEFAULT_ATUALIZADO_EM = ZonedDateTime.ofInstant(Instant.ofEpochMilli(0L), ZoneOffset.UTC).toInstant();
    private static final Instant UPDATED_ATUALIZADO_EM = ZonedDateTime.now(ZoneId.systemDefault()).withNano(0).toInstant();
